...
export default function ReadERC20(props: Props) {
  ...
  async function transfer(event: React.FormEvent) {
    ...
    const balance: BigNumber = await erc20.balanceOf(currentAccount);
    const decimals = await erc20.decimals();
    if (balance.lt(ethers.utils.parseUnits(amount, decimals))) {
      window.alert("insuficient balance")
      return
    }
    erc20.connect(signer).transfer(toAddress, ethers.utils.parseUnits(amount, decimals))
